...
labels = ["Заданая функция", "Интерполяция", "Аппроксимация"]
colorList = ["b", "r", "g", "m"]
...
for i in range(len(mas)):
    ...
    if(i >= 1):
        ...
        minY.append(min(y))
    if(i == 0):
        ax.plot(x, y, "h-b", linestyle="--", label = labels[i % len(labels)])
    else:
        ax.plot(x, y, color = colorList[i%len(colorList)], label = labels[i % len(labels)])
